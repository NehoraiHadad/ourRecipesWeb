...
### שלב G — אימות וסגירה
- [x] G1 `vitest` 426/426 + `tsc --noEmit` + `next build` ירוקים; round-trip עובר.
      עמודות `ingredients`/`formatted_content`/`recipe_metadata` הוסרו מהסכמה
      (drop ב-DB בפועל — אחרי הפריסה).
- [x] G1.5 backfill הורץ מול פרודקשן: dry-run → apply → אימות ב-SQL ישיר —
      205 מתכונים, 125 עם `ingredients_list` מובנה, 114 `is_parsed`, 204 פעילים,
      אפס מעברי `is_parsed` (הפרסר החדש תואם את הקיים). ריצה חוזרת אידמפוטנטית.
- [x] G2 נפרס ל-production (קומיט `04730aa`, deploy READY) ואומת בדפדפן על
      `recipes.nehoraihadad.com`: חיפוש ✓, מתכון מובנה (מצרכים/הוראות/צ'יפים/זמן/קושי) ✓,
      מכפיל מנות 1.5X עם שברים עבריים ("שלושת רבעי", "1 וחצי") ✓, fallback גולמי ✓,
      עמוד מקומות ✓, עמוד תפריטים (route הסריאלייזר החדש) ✓, אפס שגיאות קונסול.
      לא אומת ויזואלית: עמוד הניהול והמחיקה (הסשן בדפדפן ללא הרשאת עריכה) — מכוסה
      ב-6 בדיקות אינטגרציה; עריכה בפרודקשן לא הופעלה בכוונה (כותבת לערוץ האמיתי).
      הערת דאטה: חלק מהמתכונים החופשיים מכילים "רמת קושי: undefined" בתוך
      `raw_content` עצמו — נזק היסטורי של `buildRecipeText` הישן; יטופל בסשן
      ההמרה של המתכונים החופשיים.
- [x] G2.5 אחרי שהפריסה חיה: `prisma db push --accept-data-loss` הסיר את
      `ingredients` / `formatted_content` / `recipe_metadata` מה-DB בפרודקשן.
- [x] G3 מסמך זה עודכן; `DEPLOYMENT_TASKS.md` — נוספה רשומת הריפקטור.
...
| 5 | H-dbfirst (Sonnet) | H1–H2 | ✅ |
| 6 | ראשי | G1–G3 + backfill + drop עמודות | ✅ |
