...
- [ ] 👤 3.3 **פתוח — קריטי**: הבוט `ourRecipes_bot` **אינו חבר בערוץ הראשי** ("המתכונים שלנו - WEB"). להוסיפו כאדמין עם Post + Edit + Delete. בלעדיו: אין קליטת מתכונים חדשים מהערוץ, אין שיקוף עריכות מהאפליקציה, ובדיקת הרשאות העריכה (`getChatMember`) נכשלת לכל המשתמשים
...
- [ ] 👤 4.5 בדיקת קצה-לקצה: אחרי 3.3 — לפרסם מתכון בערוץ הראשי → מופיע בחיפוש תוך שניות
- [ ] 👤 4.6 בדיקת login: כניסה עם Telegram Login Widget + וידוא הרשאות עריכה לאדמין (דורש 3.3)
