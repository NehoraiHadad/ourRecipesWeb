...
- [x] 🤖 4.9 **סיווג הודעות ערוץ (מתכון/מקום/תפריט) + ניקוי נתונים** (קומיטים `94eab4d`, `045d5d3`):
  - **תיקון תצוגה**: `recipeMapper` שולח `details` = כל ה-`raw_content` (כמו Flask) — בלי שורת הכותרת המנותקת שגרמה לכל מתכון ליפול לתצוגת טקסט גולמי. אומת בדפדפן בפרודקשן: מודל מתכון מציג צ'קליסט מצרכים, שלבים ממוספרים, תגי קטגוריה, ובלי שורת "רמת קושי: undefined"
  - **מנגנון חדש**: `lib/telegram/messageKind.ts` מסווג לפי שורת הכותרת (`המלצה`/`תפריט חדש`), ו-`lib/telegram/channelIngest.ts` — משפך יחיד: מתכון → טבלת מתכונים; מקום → `lib/places/ingest.ts` (פירוק פורמט השיקוף, `לא צוין`→null); תפריט → דילוג. כל מסלול לא-מתכון מוחק שורת מתכון תועה (ריפוי עצמי). ה-webhook ו-upsert הפנימי עברו למשפך. 10 בדיקות יחידה חדשות
  - **ניקוי**: 6 הודעות שיובאו כמתכונים הוזנו מחדש דרך המשפך → 210→204 מתכונים, 5 מקומות נוצרו בטבלת places (טעמה, קפה יאני, הרים, נאיה, כפרית הטאבון), הודעת תפריט אחת (504) הוסרה
  - **שאריות ידועות**: 6 מתכונים עם שורת "רמת קושי: undefined" ב-raw_content (462,393,294,277,250,247 — התצוגה המובנית מסתירה אותה); 5 הודעות שירות בנות מילה (פסח ×4, חדש) נשארו כמתכונים לא-מנותחים — אפשר לארכב עם 🗑️ בערוץ
